/**
 * Example CLI demonstrating how @libria/scaffold would consume
 * ScaffoldTemplatePlugin plugins.
 *
 * Each template becomes a subcommand under `new`, with its options
 * resolved iteratively — the plugin controls which options appear
 * at each phase via getOptions().
 *
 * Phase 1: getOptions({name}) → returns initial options (e.g. {version})
 * Phase 2: getOptions({name, version: '21'}) → returns all version-relevant options
 * Repeat until no new options appear.
 *
 * This means the plugin decides what to show and when — no hardcoded
 * "cliVersion" or special-cased option names in the scaffold CLI.
 *
 * Usage:
 *   npx tsx example.ts new angular my-app --version 21 --style scss --prefix lb
 *   npx tsx example.ts new angular my-app                     # prompts for missing
 *   npx tsx example.ts new angular my-app --help              # shows initial options only
 *   npx tsx example.ts new angular my-app --version 18 --help # shows v18-specific options
 *   npx tsx example.ts new ts-lib my-lib --author "John"
 */
...
// ── Types (would come from @libria/scaffold) ────────────────────────────────
...
type ResolvedOptions<TOpt extends object> = {
    [k in keyof TOpt]: TOpt[k] extends ScaffoldTemplatePluginOption<infer TValue> ? TValue : never;
};

type ExecuteOptions<TOpt extends object> = ScaffoldTemplatePluginOptions & ResolvedOptions<TOpt>;

interface ScaffoldTemplatePlugin<TOpt extends object = object> {
    readonly argument: string;

    getOptions(
        options: ScaffoldTemplatePluginOptions & Partial<ResolvedOptions<TOpt>>,
    ): Promise<Partial<{ [k in keyof TOpt]: ScaffoldTemplatePluginOption }>>;
...
async function promptSelect(message: string, choices: unknown[], defaultValue?: unknown): Promise<string> {
...
/**
 * Extract the long flag name from a Commander flags string.
 * e.g. '--version <version>' → 'version', '--skip-git' → 'skipGit'
 */
function parseFlagName(flags: string): string | null {
    const match = flags.match(/--([a-z][a-z-]*)/i);
    if (!match) return null;
    // Convert kebab-case to camelCase (Commander convention)
...
const SCAFFOLD_TEMPLATE_TYPE = 'scaffold-template';

async function registerNewCommand(program: Command, pluginManager: PluginManager): Promise<Command> {
    const newCmd = program
        .command('new')
        .description('Create a new project from a template');

    const templates = pluginManager.getPluginsByType(SCAFFOLD_TEMPLATE_TYPE);
    ...
    for (const meta of templates) {
        ...
        let optionDefs = await plugin.getOptions(
            {name: ''} as ScaffoldTemplatePluginOptions & Partial<Record<string, unknown>>,
        );
...
async function main() {
    ...
    // const pluginsDir = (process.env.SCAFFOLD_PLUGINS ?? './plugins').replace(/\\/g, '/');
    // await pluginManager.loadPlugins([`${pluginsDir}/**`]);
    await pluginManager.loadPlugins([
        '../angular/.tmp-clean-publish',
        '../ts-lib/.tmp-clean-publish'
    ]);
